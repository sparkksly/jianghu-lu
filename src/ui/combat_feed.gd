...
# Returns {} or {lane:int, text:String, tone:String}. tone in {"big","good","bad"}.
# `lane` is the player index whose lane the marker sits in.
static func marker(e) -> Dictionary:
	match e.type:
		&"hit":
			if Loc.is_combo_result(e.move_id):
				return {"lane": e.actor, "text": "大招", "tone": "big"}
			if e.amount >= HEAVY_DMG:
				return {"lane": e.actor, "text": "重击", "tone": "big"}
			return {}
		&"interrupt":
			return {"lane": e.actor, "text": "打断", "tone": "big"}
		&"throw_break":
			return {"lane": e.actor, "text": "破防", "tone": "big"}
		&"block":
