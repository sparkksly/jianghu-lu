...
const EVOLVE_AT := [3, 6]
# 武学境界(人物等级):单 run,经验驱动。升境界涨基础属性(主线) + 解锁能力(抽牌数)。
const LEVEL_NAMES := ["", "三流", "二流", "一流", "绝顶", "先天"]
const MAX_LEVEL := 5
const XP_THRESHOLDS := [0, 30, 75, 140, 230]   # 升到 level(下标)的累计经验
const LEVEL_GAINS := {
	2: {"hp": 6, "qi": 1, "atk": 1, "hand": 0},
	3: {"hp": 6, "qi": 1, "atk": 1, "hand": 1},
	4: {"hp": 8, "qi": 1, "atk": 2, "hand": 0},
	5: {"hp": 10, "qi": 1, "atk": 2, "hand": 1},
}
const BASE_HAND := 6   # 起手抽牌数(进攻牌)
...
var level: int = 1                  # 武学境界(1 三流 .. 5 先天)
var xp: int = 0                     # 当前累计经验
var hand_bonus: int = 0             # 境界解锁的额外抽牌数
...
func level_name() -> String:
	return LEVEL_NAMES[clampi(level, 1, MAX_LEVEL)]
...
func hand_size() -> int:
	return BASE_HAND + hand_bonus

# 获得经验,够则升境界(可连升);升级涨基础属性主线 + 解锁抽牌数。返回升了几级。
func gain_xp(n: int) -> int:
	xp += n
	var ups := 0
	while level < MAX_LEVEL and xp >= XP_THRESHOLDS[level]:
		level += 1
		ups += 1
		var g: Dictionary = LEVEL_GAINS[level]
		max_hp += int(g["hp"]); player_hp += int(g["hp"])   # 升境界回血+涨上限
		base_max_qi += int(g["qi"])
		base_attack += int(g["atk"])
		hand_bonus += int(g.get("hand", 0))
	return ups
